...
import { ModalTask } from "../ModalForm/modalFormTask";
...
import { Confirm } from "../ModalForm/confirmFormDelete";
...
import { ModalCategory } from "../ModalForm/modalFormCategory";
...
export const Table: React.FC<Props> = () => {
  ...
  const globalState: AppState.globalState = useSelector(
    (state: RootStore) => state.global
  );
  ...
  const databaseLoader = () => {
    database.getAllObjects<(result: AppState.itemState[]) => any>(
      "tasks",
      (res) => dispatch(appImportTask(res))
    );
    database.getAllObjects<(result: AppState.categoryState[]) => any>(
      "category",
      (res) => dispatch(appImportCategory(res))
    );
  };
  ...
  return (
    <div>
      {globalState.selectMode == "tasks" ? (
        <ModalTask active={modalMod} setActive={toggleModal} data={data} />
      ) : (
        <ModalCategory active={modalMod} setActive={toggleModal} data={data} />
      )}
          ...
          {(globalState.selectMode === "tasks"
            ...
            return (
              <tr key={item.id}>
                ...
                {categoryState.data.map((item1: AppState.categoryState) => {
                  if (item.CategoryId == item1.id) {
                    return <td>{item1.Name}</td>;
                  }
                  return "";
                })}
